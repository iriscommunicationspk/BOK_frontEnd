...
const data1 = ref({
  series: [Math.floor(dataStore.branch_interior_overAllScore)],
  chartOptions: computed(() => ({
    ...
    fill: {
      colors: [getColor(dataStore.branch_interior_overAllScore)]
    },
    labels: [' ']
  }))
})
...
watchEffect(() => {
  data1.value.series = [Math.floor(dataStore.branch_interior_overAllScore)]
  data1.value.chartOptions.fill.colors = [getColor(dataStore.branch_interior_overAllScore)]
})
</script>
