...
const data1 = ref({
  series: [Math.floor(dataStore.customer_service_overAllScore)],
  chartOptions: computed(() => ({
    ...
    fill: {
      colors: [getColor(dataStore.customer_service_overAllScore)]
    },
    labels: [' ']
  }))
})
...
watchEffect(() => {
  data1.value.series = [Math.floor(dataStore.customer_service_overAllScore)]
  data1.value.chartOptions.fill.colors = [getColor(dataStore.customer_service_overAllScore)]
})
</script>
...
<template>
  ...
  >
    <div>
      ...
      >
        Customer Service Overall Score
      </h4>
    </div>
    <div class="relative">
