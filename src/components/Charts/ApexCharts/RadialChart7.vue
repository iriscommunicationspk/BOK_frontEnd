...
const data1 = ref({
  series: [Math.floor(dataStore.atm_services_overAllScore)],
  chartOptions: computed(() => ({
    ...
    fill: {
      colors: [getColor(dataStore.atm_services_overAllScore)]
    },
    labels: [' ']
  }))
})
...
watchEffect(() => {
  data1.value.series = [Math.floor(dataStore.atm_services_overAllScore)]
  data1.value.chartOptions.fill.colors = [getColor(dataStore.atm_services_overAllScore)]
})
</script>
