...
const data1 = ref({
  series: [Math.floor(dataStore.cash_counter_overAllScore)],
  chartOptions: computed(() => ({
    ...
    fill: {
      colors: [getColor(dataStore.cash_counter_overAllScore)]
    },
    labels: [' ']
  }))
})
...
watchEffect(() => {
  data1.value.series = [Math.floor(dataStore.cash_counter_overAllScore)]
  data1.value.chartOptions.fill.colors = [getColor(dataStore.cash_counter_overAllScore)]
})
</script>
