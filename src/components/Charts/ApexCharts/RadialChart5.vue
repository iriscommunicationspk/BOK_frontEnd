...
const data1 = ref({
  series: [Math.floor(dataStore.product_knowledge_overAllScore)],
  chartOptions: computed(() => ({
    ...
    fill: {
      colors: [getColor(dataStore.product_knowledge_overAllScore)]
    },
    labels: [' ']
  }))
})
...
watchEffect(() => {
  data1.value.series = [Math.floor(dataStore.product_knowledge_overAllScore)]
  data1.value.chartOptions.fill.colors = [getColor(dataStore.product_knowledge_overAllScore)]
})
</script>
